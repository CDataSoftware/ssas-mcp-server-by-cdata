...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our [CData MCP Server for SQL Server Analysis Services](https://www.cdata.com/drivers/ssas/download/mcp/). 
## Purpose
...
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        ...
        * (Mac/Linux) `/Applications/CData JDBC Driver for SQL Server Analysis Services/`
    * Run the command `java -jar cdata.jdbc.ssas.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.ssas.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `ssas.prp`) using the following properties and format:
